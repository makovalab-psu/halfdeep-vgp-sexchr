...
do
    ...
    wrapper_cmd="sbatch -o \"$LOGFILE\" -e \"$LOGFILE\" halfdeep_wrapper_sra.sh \"$data_no\" \"$species\" \"$accession\" \"$fasta_name\" \"$assembly_id\" \"$read_db\" \"$tech\" $file_list"
    ...
    while true; do
        ...
        if [[ $count -ge 7 ]]; then
            echo "Waiting: halfdeep jobs >= 6 ($count running/pending)"
            sleep 60
        else
            break
        fi
