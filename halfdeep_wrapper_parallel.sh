...
if [ "$db" == "AWS" ]; then
    ...
    echo "Read must be taken from NCBI: developing.."
fi
...
if ls "$read_path" | grep -E '\.(fastq|fastq\.gz|fq|fq\.gz)$' > /dev/null; then # Fix 07022025 # found a bug. second block never reached. (fasta also inclued here. if some species only have a fasta.. 3 hifi read species have this issue: manual checking... -> confirm to check: no problem because that 3 species are not BIG file species -> run for wrapper not wrapper_parallel. wrapper dosen't have this code block. if Big file species have only fasta files, that no read )
    echo "Found fastq. Skipping bam2fasta. Only merge"
    echo "[[ ALERT ]]There might be a bug here: a big file species only have fasta "
